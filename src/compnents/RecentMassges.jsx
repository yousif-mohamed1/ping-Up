...
import { api, normalizeMessage, normalizeUser } from '../services/api'
        ...
        <div className='flex flex-col max-h-56 overflow-y-scroll no-scrollbar'>
            {
                massages.map((massage) => {
                    const sender = normalizeUser(massage.sender || massage.from_user_id || {})
                    return (
                    <div key={massage._id} className='flex items-start gap-2 py-2
                    hover:bg-slate-100 cursor-pointer' onClick={() => navigate(`/messages/${sender._id}`)}>
                        <img src={sender.profile_picture} alt="" className='w-8 h-8 rounded-full object-cover shrink-0' />
                        ...
                        <div className='flex justify-between'>
                            ...
                            text-white w-4 h-4 flex items-center justify-center rounded-full text-[10px]'>1</p1>}
                        </div>
                        </div>
